...
export default class Board {
        ...
        for (const from in moves) {
            moves[from].map(to => {
                ...
                scoreTable.push({
                    from,
                    to,
                    score: testBoard.testMoveScores(this.getPlayingColor(), this.getAIMaxDepth(level)).score +
    ...
    testMoveScores (playingPlayerColor, level, depth = 0) {
        let nextMoves = {}
        if (this.hasPlayingPlayerCheck()) {
        ...
        for (const from in nextMoves) {
            ...
            nextMoves[from].map(to => {
                ...
                const result = testBoard.testMoveScores(playingPlayerColor, level, depth + 1)
                if (result.checkMate) {
