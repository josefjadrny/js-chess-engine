...
export default class Board {
    ...
    calculateAiMove (level) {
        const bestMove = { score: this.calculateScore(), from: null, to: null }
        ...
        for (const from in this.playingPlayer.moves) {
            this.playingPlayer.moves[from].map(to => {
                ...
                const scoreIndex = testBoard.calculateScore()
                if (bestMove.from === null || bestMove.score > scoreIndex) {
                    Object.assign(bestMove, { from, to, score: scoreIndex })
                }
            })
        }
        return bestMove
    ...
    calculateScore () {
        let scoreIndex = 0
        this.playingPlayer.chessMen.map(chessman => {
            if (chessman.isInGame()) {
                scoreIndex += chessman.getValue()
            }
        })
        this.getNonPlayingPlayer().chessMen.map(chessman => {
            if (chessman.isInGame()) {
                scoreIndex -= chessman.getValue()
            }
        })
