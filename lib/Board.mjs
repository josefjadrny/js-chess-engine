...
export default class Board {
    ...
    createFromJson (jsonConfig = {}) {
        ...
        if (jsonConfig.enPassant && this.isLocationValid(jsonConfig.enPassant)) {
            this.enPassant = jsonConfig.enPassant
        }
        return this
    }
    ...
    createFromFEN (fen = '') {
        ...
        for (let index = 0; index < pieces.length; index++) {
            console.log(`${COLUMNS[columnIndex]}${ROWS[rowIndex]}`, pieces[index])
            switch (pieces[index]) {
            case 'K': this.playerWhite.addKing(`${COLUMNS[columnIndex]}${ROWS[rowIndex]}`); columnIndex++; break
            case 'Q': this.playerWhite.addQueen(`${COLUMNS[columnIndex]}${ROWS[rowIndex]}`); columnIndex++; break
            case 'R': this.playerWhite.addRook(`${COLUMNS[columnIndex]}${ROWS[rowIndex]}`); columnIndex++; break
            case 'B': this.playerWhite.addBishop(`${COLUMNS[columnIndex]}${ROWS[rowIndex]}`); columnIndex++; break
            case 'N': this.playerWhite.addKnight(`${COLUMNS[columnIndex]}${ROWS[rowIndex]}`); columnIndex++; break
            case 'P': this.playerWhite.addPawn(`${COLUMNS[columnIndex]}${ROWS[rowIndex]}`); columnIndex++; break
            case 'k': this.playerBlack.addKing(`${COLUMNS[columnIndex]}${ROWS[rowIndex]}`); columnIndex++; break
            case 'q': this.playerBlack.addQueen(`${COLUMNS[columnIndex]}${ROWS[rowIndex]}`); columnIndex++; break
            case 'r': this.playerBlack.addRook(`${COLUMNS[columnIndex]}${ROWS[rowIndex]}`); columnIndex++; break
            case 'b': this.playerBlack.addBishop(`${COLUMNS[columnIndex]}${ROWS[rowIndex]}`); columnIndex++; break
            case 'n': this.playerBlack.addKnight(`${COLUMNS[columnIndex]}${ROWS[rowIndex]}`); columnIndex++; break
            case 'p': this.playerBlack.addPawn(`${COLUMNS[columnIndex]}${ROWS[rowIndex]}`); columnIndex++; break
            case '1': columnIndex++; break
            case '2': columnIndex = columnIndex + 2; break
            case '3': columnIndex = columnIndex + 3; break
            case '4': columnIndex = columnIndex + 4; break
            case '5': columnIndex = columnIndex + 5; break
            case '6': columnIndex = columnIndex + 6; break
            case '7': columnIndex = columnIndex + 7; break
            case '8': columnIndex = columnIndex + 8; break
            case '/': rowIndex--; columnIndex = 0; break
            }
        }
        ...
        if (castlings.includes('K')) {
            this.castlings.whiteShort = true
        }
        if (castlings.includes('k')) {
            this.castlings.blackShort = true
        }
        if (castlings.includes('Q')) {
            this.castlings.whiteLong = true
        }
        if (castlings.includes('q')) {
            this.castlings.blackLong = true
        }

        // enPassant
        if (this.isLocationValid(enPassant)) {
            this.enPassant = enPassant
        }

        // halfmoves
        this.counters.halfMove = parseInt(halfmove)

        // fullmoves
        this.counters.fullMove = parseInt(fullmove)
    ...
    move (from, to, recalculate = true) {
        ...
        // En passant check
        if (chessmanFrom.isPawn() && to === this.enPassant) {
            const pawnToRemoveField = fieldTo.down(this.playingPlayer.color)
            pawnToRemoveField.chessMan.field = null
            pawnToRemoveField.chessMan = null
        }
