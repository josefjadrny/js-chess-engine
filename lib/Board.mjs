...
export default class Board {
        ...
        for (const from in this.playingPlayer.moves) {
            this.playingPlayer.moves[from].map(to => {
                ...
                const moveStatus = {
                    ...
                    totalScore: scoreTable.reduce((total, score) => { return total + score }),
    ...
    testMoveScores (from, to, playingPlayerColor, level, scoreTable) {
        ...
        while (level > 0) {
            ...
            if (testBoard.hasPlayingPlayerCheck()) {
                testBoard.recalculate()
                if (testBoard.checkMate) {
                    scoreTable.push(testBoard.calculateScore(playingPlayerColor))
                }
            }
            const movesToCalculate = testBoard.playingPlayer.getMoves()
            for (const fromNested in movesToCalculate) {
                movesToCalculate[fromNested].map(toNested => {
        ...
        if (level === 0) {
            ...
            let score = testBoard.calculateScore(playingPlayerColor)

            this.playingPlayer.color === playingPlayerColor
                ? score += this.playingPlayer.getHighestValueAttackingPiece()
                : score -= this.getNonPlayingPlayer().getHighestValueAttackingPiece()
