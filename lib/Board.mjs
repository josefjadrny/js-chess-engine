    ...
    calculateAiMove (level) {
        ...
        for (const from in this.playingPlayer.moves) {
            this.playingPlayer.moves[from].map(to => {
                ...
                const moveStatus = {
                    ...
                    minScore: scoreTable.reduce((minScore, score) => { return score + random < minScore ? score + random : minScore })
                }
        ...
        scores.sort((previous, next) => {
            if (previous.minScore === next.minScore) {
                return previous.avgPlusScore < next.avgPlusScore ? 0 : -1
            } else {
