...
export default class Board {
        ...
        for (const from in this.playingPlayer.moves) {
            this.playingPlayer.moves[from].map(to => {
                scoreTable.push({
                    from,
                    to,
                    score: this.testMoveScores(from, to, playingPlayerColor, level) + Math.floor(Math.random() * Math.floor(10)) / 10,
                })
            })
        }
    ...
    testMoveScores (from, to, playingPlayerColor, level, depth = 0) {
        ...
        const maxLevel = level + (this.playingPlayer.chessMen.length < 9 ? 1 : 0)

        if (depth < maxLevel || this.hasPlayingPlayerCheck()) {
            testBoard.recalculate()
        }

        if (depth >= maxLevel || testBoard.checkMate) {
            ...
            if (chessman.isInGame()) {
                ...
                chessman.getMoves().map(move => {
                    if (this.getChessman(move) && this.getChessman(move).color !== playerColor) {
                        scoreIndex += this.getChessman(move).getValue()
                    }
                })
            }
        })
