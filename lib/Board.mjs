...
export default class Board {
        ...
        for (const from in moves) {
            moves[from].map(to => {
                scoreTable.push({
                    from,
                    to,
                    score: this.testMoveScores(from, to, this.getPlayingColor(), this.getAIMaxDepth(level)).score + Math.floor(Math.random() * Math.floor(2)),
                })
            })
        }
    ...
    testMoveScores (from, to, playingPlayerColor, level, depth = 0) {
        ...
        if (depth >= level || testBoard.configuration.isFinished) {
            return {
                score: testBoard.calculateScore(playingPlayerColor) + (testBoard.getPlayingColor() === playingPlayerColor ? depth : -depth),
                checkMate: testBoard.configuration.checkMate,
            }
        }
        ...
        for (const fromNested in nextMoves) {
            ...
            nextMoves[fromNested].map(toNested => {
                ...
                const result = testBoard.testMoveScores(fromNested, toNested, playingPlayerColor, level, depth + 1)
                if (result.checkMate) {
